...
public class PointDeVente {
	...
	/** Obtenir le prix d'un carburant à une date donnée.  Si ce point
	 ...
	 * @param date la date où l'on veut connaître le prix
	 */
	public int getPrix(Carburant carburant, LocalDateTime date) {
		int prix = 0;
		if (this.prix.containsKey(carburant)) {
			Entry<LocalDateTime, Integer> date2 = this.prix.get(carburant).floorEntry(date);
			if (date2 != null) {
				prix = date2.getValue();
			}
		}
