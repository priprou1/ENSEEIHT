...
%
    ...
    res = donnees(:,2) - (beta(1)* exp(-donnees(:,1)*beta(2)));
...
%
    ...
    res=residu(beta);
    J_res=J_residu(beta);
    %H_f = [0 , donnees(:,1).*exp(-beta(2)*donnees(:,1)) ; donnees(:,1).*exp(-beta(2)*donnees(:,1)) , (-(donnees(:,1).^2).*donnees(:,2)).*exp(-beta(2)*donnees(:,1))];
    H_f=J_res'*J_res;
   
    nb_obs=size(res,1);
    for i=1:nb_obs
        H_res= [0 , donnees(i,1)*exp(-beta(2)*donnees(i,1)) ; donnees(i,1)*exp(-beta(2)*donnees(i,1)) , (-(donnees(i,1)^2)*beta(1))*exp(-beta(2)*donnees(i,1))];
        H_f=H_f+res(i)*H_res;
    end
