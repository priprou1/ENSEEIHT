...
intro e.
destruct (H2 e).  (*permet d'appliquer l'hypothèse H2 sur l'élément e qui est dans l'hypothèse*)
destruct (H2 x).

exists x0.
apply H3. (* A->B prouver A revient à A->B prouver B pour montrer que c'est vrai*)
exists x.
split.
exact H0.
exact H1.
