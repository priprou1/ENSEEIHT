...
public class MaxTabThread {
    ...
    private static int computeMax(int[] array, int numberOfThreads) throws InterruptedException {
        ...
        while (startIndex < array.length) {
            endIndex = Math.min(array.length, startIndex + taskSize);
            PartialMax partialMax = new PartialMax(array, startIndex,endIndex,results);
            Thread t = new Thread(partialMax);
            threads.add(t);
            t.start();
            startIndex = endIndex;
        }
