# Problème Affectation de commandes de clients au magasin cas 2
...
var quantitefluide{i in MAGASIN, j in FLUIDE}, integer, >=0; # matrice de quantité de fluide j pris dans le magasin i
...
printf " magasins cas linéaire en nombre entier\n";
...
printf "Coût Total : %d\n", CoutTotal;
printf{j in FLUIDE, i in MAGASIN} "Quantité de fluide %s pris dans le magasin %s : %d\n", j, i, quantitefluide[i,j];
printf "\n";
