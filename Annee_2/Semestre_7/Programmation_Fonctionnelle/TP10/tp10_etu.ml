...
let rec parse_P : (char, prog) parser = fun flux ->
  ( p_begin >>= fun () -> parse_I >>= fun inst -> p_end >>= fun () -> return inst
  ) flux
  (* ou cette syntaxe :
  let* () = p_begin in
  let* inst = parse_I in
...
and parse_C : (char, cmd) parser = fun flux ->
  ( (p_repeat >>= fun () -> p_entier >>= fun i -> parse_P >>= fun prog -> return (Repeat (i,prog)))
  ++
  (p_move >>= fun () -> p_entier >>= fun i -> return (Move i))
  ++
  (p_turn >>= fun () -> p_entier >>= fun i -> return (Turn i))
  ++
  (p_on >>= fun () -> return On)
  ++
...
let main nom =
  let flux = flux_of_file nom in
  let progs = parse_logo flux in 
  match Solution.uncons progs with
    | None        -> (Format.printf "** No program ! **@.")
    | Some (prog, _) -> run_logo prog
