...
module FreeFall (F : Frame) =
  struct
    ...
    let run : etat -> etat Flux.t =  fun ((x,y),(x',y')) ->
    let acceleration = Flux.constant(0., -98.1) in
    let vitesse = Flux.map(fun (ax,ay) -> (ax +. x', ay +. y')) (integre F.dt acceleration) in
    let position = Flux.map(fun (ax,ay) -> (ax +. x, ay +. y)) (integre F.dt vitesse) in
    Flux.map2(fun x y -> (x,y)) position vitesse
...
let rec unless : 'a Flux.t -> ('a -> bool) -> ('a -> 'a Flux.t) -> 'a Flux.t =
  fun flux condition restart -> 
    Tick(lazy(match Flux.uncons flux with
    | None -> None
    | Some (t,q) -> if condition t then Flux.uncons(restart t) else Some(t,unless q condition restart)
    ))
...
struct
  let contact_x ((x,_),(x',_)) =
    let (xmin, xmax) = F.box_x in
    (x<=xmin && x' <=0.) || (x>=xmax && x'>=0.)
  let contact_y ((_,y),(_,y')) =
    let (ymin, ymax) = F.box_y in
    (y<=ymin && y' <=0.) || (y>=ymax && y'>=0.)

  let rebound etat = 
    match etat with
    | ((x,y),(x',y')) -> let x' =
    if contact_x etat then -. x' else x' in
    let y' =
    if contact_y etat then -. y' else y' in
    ((x,y),(x',y'))
    module FF = FreeFall(F)
      
    let rec run etat =
      unless(FF.run etat) (fun etat -> contact_x etat || contact_y etat) (fun etat -> run (rebound etat))
  end

(*Exo 1:*)
(*module FF = Bouncing(F)*)
(*Exo 6*)
module FF = Bouncing(F)
module D = Drawing(F)
...
let _ =
  let (x0,y0) = (20.,300.) in
  let (x0',y0') = (100., 100.) in
  D.draw(FF.run ((x0,y0),(x0',y0')))
