...
(******************************************************************************)
let trie_dico (Trie(arbre, _, recompose)) = List.map recompose (parcours arbre)
...
let affiche p (Trie(arbre, decompose, recompose)) = 
  let td = trie_dico (Trie(arbre, decompose, recompose)) 
  in let rec affiche_aux list =
    match list with
    | [] -> ()
    | h::t -> p h; affiche_aux t
  in affiche_aux td
