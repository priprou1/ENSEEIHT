...
(******************************************************************************)
(*   fonction de retrait d'une liste éléments d'un arbre                      *)
...
(******************************************************************************)
(*   fonction de parcours d'un arbre                                          *)
(*   signature  : parcours_arbre : 'a arbre -> 'a list list                   *)
(*   paramètres : - un arbre n-aire                                           *)
(*   résultat   : la liste des éléments dans l'ordre lexicographique          *)
(******************************************************************************)
let parcours (Noeud (b,lb)) =
   let rec parcours_aux (Noeud(b,lb)) liste mots =
      let nliste = if b then mots::liste else liste in 
      match lb with
      | [] -> nliste
      | _ -> 
         List.fold_left (fun lst (a, arbre) -> parcours_aux arbre lst (List.append mots [a])) nliste lb
   in List.rev (parcours_aux (Noeud(b,lb)) [] [])
...
let rec normalisation (Noeud(b,lb)) =
   match lb with
   | [] -> Noeud (b, [])
   | _ -> Noeud(b, (List.filter (fun (_,Noeud(b,lb)) -> b || not (lb = [])) (List.map (fun (a,arbre) ->(a,normalisation arbre)) lb)) )